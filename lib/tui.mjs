...
const permissionDialogWidthSchema = z.number().step(1).min(20).default(64);
const permissionDialogMaxHeightSchema = z.number().step(1).min(4).default(10);
...
const tuiConfigSchemaFields = {
	...
	permissionDialogWidth: permissionDialogWidthSchema,
	permissionDialogMaxHeight: permissionDialogMaxHeightSchema,
...
function resolveTuiConfig(config) {
	return {
		...
		permissionDialogWidth: config?.permissionDialogWidth ?? 64,
		permissionDialogMaxHeight: config?.permissionDialogMaxHeight ?? 10,
	...
	return {
		...
		backgrounds: {
			panel: {
				open: "100",
				close: "49",
				purpose: "Message-body panels and cards"
			},
			accentBg: {
				open: "105",
				close: "49",
				purpose: "User-message block bar"
			},
			successBg: {
				open: "102",
				close: "49",
				purpose: "Succeeded tool block bar"
			},
			warningBg: {
				open: "103",
				close: "49",
				purpose: "Authorization-pending tool block bar"
			},
			errorBg: {
				open: "101",
				close: "49",
				purpose: "Failed tool block bar"
			}
		},
		attributes: {
			bold: {
...
function renderPalette(palette, scheme, colorEnabled) {
	...
	for (const name of BACKGROUND_ROLES) rows.push(head(name, spec.backgrounds[name], palette[name](PALETTE_SAMPLE)), purpose(spec.backgrounds[name]));
...
*/
function shimmerTitle(palette, text, frame, width) {
	...
	for (let index = 0; index < chars.length; index += 1) {
		...
		row += column >= start && column < start + SHIMMER_WINDOW_COLUMNS ? palette.italic(palette.bold(char)) : palette.italic(palette.dim(char));
	}
...
*/
var ShimmerTitleComponent = class {
	palette;
	title;
	shimmer;
	constructor(palette, title, shimmer) {
		this.palette = palette;
		this.title = title;
...
* 1 border + 2 padding).
*/
function blockLayout(width, options) {
	const slot = !options.panel ? 1 : options.bar !== "none" ? options.barWidth : 0;
	const padX = Math.min(options.paddingX, Math.max(0, Math.floor((width - slot - 1) / 2)));
	return {
		padX,
		innerWidth: Math.max(1, width - slot - padX * 2)
	};
}
/** The palette background that paints a block's visible gutter bar. */
function barBackground(palette, bar) {
...
*/
function paintBlock(palette, body, width, layout, options) {
	...
	const bar = options.bar === "none" ? void 0 : barBackground(palette, options.bar);
	const barColumns = bar === void 0 ? 0 : options.barWidth;
	...
	const paint = (content) => {
		...
		const row = bar === void 0 ? panel : `${bar(" ".repeat(barColumns))}${panel}`;
		return bar === void 0 ? row : truncateToWidth(row, width, "");
	};
	for (let index = 0; index < options.paddingY; index += 1) rows.push(paint(""));
	for (const line of body) rows.push(paint(`${" ".repeat(layout.padX)}${truncateToWidth(line, layout.innerWidth, "")}`));
	for (let index = 0; index < options.paddingY; index += 1) rows.push(paint(""));
	return rows;
}
/**
...
*/
var MessageBlock = class extends Container {
	palette;
	options;
...
* them statically.
*/
function assistantMessageChildren(content, reasoningMode, foldedContinuation, palette, mdTheme, live) {
	...
	if (showsReasoning) {
		const title = (marker, label) => live?.streaming === true && live.shimmer !== void 0 ? new ShimmerTitleComponent(palette, `${marker} ${label}`, live.shimmer) : new Text(palette.italic(palette.dim(`${marker} ${label}`)), 0, 0);
		if (reasoningMode === "collapsed") {
			const chip = new MessageBlock(palette, {
				bar: "panel",
				paddingY: 0
			});
...
var StepTimingComponent = class extends Container {
	...
	rebuild() {
		...
		const header = this.completionTime === void 0 ? `Status · ${timing}` : `Status · ${timing} · Completed ${formatCompletionTime(this.completionTime)}`;
...
var StreamingAssistantComponent = class extends Container {
	...
	*/
	timing;
	constructor(position, events, tracker, now, reasoningMode, palette, mdTheme) {
		super();
		this.position = position;
		this.now = now;
	...
	*/
	hasVisibleBody() {
		...
		return textBlocks(content, "text").trim() !== "" || this.reasoningMode !== "hidden" && textBlocks(content, "reasoning").trim() !== "";
	}
	/**
	...
	*/
	hasLiveReasoning() {
		return this.settledContent === void 0 && this.reasoningMode !== "hidden" && textBlocks(this.presentedContent(), "reasoning").trim() !== "";
	}
	...
	rebuild() {
		this.clear();
		const children = assistantMessageChildren(this.presentedContent(), this.reasoningMode, this.foldedContinuation, this.palette, this.mdTheme, this.settledContent === void 0 ? {
			streaming: true,
			shimmer: () => shimmerFrame(this.now)
		} : void 0);
...
*/
var CachedCardComponent = class CachedCardComponent {
	/**
	...
	*/
	...
	render(width) {
		...
		if (cached !== void 0 && cached.epoch === epoch) return cached.lines;
		const lines = this.renderLines(width);
		this.cached.delete(width);
		this.cached.set(width, {
			epoch,
			lines
		});
...
var ToolCardComponent = class extends CachedCardComponent {
	...
	resultView;
	diffBodyCache;
	constructor(name, parsed, definition, maxOutputLines, maxDiffEditLength, palette, mdTheme, shimmer) {
		super();
		this.name = name;
		this.parsed = parsed;
	...
	*/
	isPending() {
		return this.result === void 0 && this.visibility !== "hidden";
	}
	cacheEpoch() {
		return this.result === void 0 && this.shimmer !== void 0 ? shimmerFrame(this.shimmer) : void 0;
	}
	presentCall() {
	...
	*/
	...
	renderLines(width) {
		...
		const options = blockOptions(this.awaitingApproval && this.result === void 0 ? {
			bar: "warning",
			barWidth: 2
		...
		const body = unknownXml ?? (markdownContent !== void 0 && rawBody.lines.length > 0 ? this.dimBody(rawBody, layout.innerWidth) : [...rawBody.prelude, ...rawBody.lines]);
		...
		const headerRest = `${displayText(this.name)}${detail === void 0 ? "" : ` / ${displayInlineText(detail)}`}`;
		const headerRestRendered = isError ? this.palette.error(headerRest) : this.result === void 0 && this.shimmer !== void 0 ? shimmerTitle(this.palette, headerRest, shimmerFrame(this.shimmer), Math.max(1, layout.innerWidth - visibleWidth(glyph) - 1)) : this.palette.dim(headerRest);
		const header = truncateToWidth(`${statusColor(glyph)} ${headerRestRendered}`, layout.innerWidth, "");
		const bodyRows = visibleBody.length > 0 ? new Text(visibleBody.join("\n"), 0, 0).render(layout.innerWidth) : [];
	...
	*/
	headerDetail() {
		...
		return title !== "" && title !== displayText(this.name) ? title : void 0;
	}
	/**
	...
	*/
	...
	renderBody() {
		...
		if (rawInput !== void 0) {
			...
			lines.push(...this.visibility === "expanded" ? rendered.split("\n") : [rendered.replace(/\s*\n\s*/gu, " ")]);
		}
...
var PermissionDialog = class {
	...
	selectedIndex = 0;
	focused = false;
	constructor(choices, palette, done, cancel) {
	...
	invalidate() {}
	handleInput(data) {
		if (matchesKey(data, Key.up)) this.selectedIndex = this.selectedIndex === 0 ? this.choices.length - 1 : this.selectedIndex - 1;
		else if (matchesKey(data, Key.down)) this.selectedIndex = this.selectedIndex === this.choices.length - 1 ? 0 : this.selectedIndex + 1;
		else if (matchesKey(data, Key.enter)) {
	...
	render(width) {
		...
		for (const [index, choice] of this.choices.entries()) {
			...
			const row = ` ${cursor} ${choice.label}${currentMark}${choice.description === void 0 ? "" : ` — ${choice.description}`}`;
...
function createCommandController(deps) {
	...
	const showHelp = () => {
		...
		deps.appendNotice([
			...
			"Ctrl+O cycle cards (collapse/expand/hide) • Ctrl+R cycle reasoning (hidden/collapsed/expanded) • Ctrl+C×2/Ctrl+D exit",
			"",
			...commandLines
		].join("\n"), "info");
	};
...
//#endregion
//#region packages/tui/src/chat/permission.ts
/** Kebab-case preset names are title-cased for terminal presentation. */
function permissionPresetLabel(value, name) {
	if (value === "danger-full-access") return "Full access";
	if (/^[a-z0-9]+(-[a-z0-9]+)*$/u.test(name)) return name.split("-").map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join(" ");
	return name;
}
/**
...
*/
function createPermissionController(deps) {
	const { ctx, agent, palette, resolved, overlayManager } = deps;
	...
	const currentSandboxMode = () => {
		...
		return ctx.get("shell")?.sandboxMode ?? ctx.get("sandboxPolicy")?.defaultMode ?? "read-only";
	};
	const appendPermissionPreset = (session, preset) => {
		session.append("permission/preset", { preset });
	};
	const appendSandboxMode = (session, mode) => {
		session.append("sandbox/mode", { mode });
	};
	const applyApprovalPolicy = (policy) => {
		if (policy === deps.currentPolicy()) {
			deps.appendNotice(`Permission mode is already ${policy}.`);
			closeSelector();
			return;
		}
		const service = approval();
		if (service?.setPolicy !== void 0) service.setPolicy(agent, policy);
		else setApprovalPolicy(agent.session, policy);
	...
	const applyPreset = (name) => {
		...
		const events = agent.session.events;
		if (service.current(events) !== name) appendPermissionPreset(agent.session, name);
		const currentSandbox = currentSandboxMode();
		if (spec.sandbox !== currentSandbox) appendSandboxMode(agent.session, spec.sandbox);
		const currentApproval = effectiveApprovalPolicy(events) ?? approval()?.config?.policy ?? "ask";
		if (spec.approval !== currentApproval) {
			const serviceApproval = approval();
			if (serviceApproval?.setPolicy !== void 0) serviceApproval.setPolicy(agent, spec.approval);
			else setApprovalPolicy(agent.session, spec.approval);
		}
	...
	const applyChoice = (value) => {
		if (presets() !== void 0) applyPreset(value);
		else applyApprovalPolicy(value);
	};
	const buildChoices = () => {
		const service = presets();
		if (service !== void 0) {
		...
		return [{
			value: "ask",
			label: "ask",
			description: "Ask before permission-sensitive actions",
			current: current === "ask"
		}, {
			value: "never",
			label: "never",
			description: "Auto-reject permission-sensitive actions",
	...
	const showSelector = () => {
		...
		const session = overlayManager.open({
			create: () => new PermissionDialog(choices, palette, (value) => {
				applyChoice(value);
			}, () => {
				closeSelector();
			}),
		...
		session.closed.then(() => {
			...
			if (permissionOverlay === session) permissionOverlay = void 0;
		});
		deps.requestRender();
	};
	return {
		queuePermissionCommand(raw) {
			permissionCommands = permissionCommands.then(() => {
				...
				const service = presets();
				if (service !== void 0) {
					if (service.names.includes(argument)) applyPreset(argument);
					else deps.appendNotice(`Unknown permission mode "${argument}". Usage: /permission [${service.names.join("|")}]`, "warning");
					return;
				}
				if (APPROVAL_POLICIES.includes(argument)) {
					applyApprovalPolicy(argument);
					return;
				}
...
function createResumeController(deps) {
	...
	*/
	const readSessionEvents = async (query, sessionId) => {
		let lastError;
		for (let attempt = 0; attempt < RESUME_READ_RETRIES; attempt += 1) try {
			return (await query.readSession(sessionId)).events;
		} catch (error) {
			lastError = error;
			if (!errorChain(error).includes("complete frame contains a torn JSONL record") || attempt === 2) throw error;
	...
	*/
	const preflightResume = async (sessionId) => {
		...
		try {
			...
			if (message.includes("complete frame contains a torn JSONL record")) throw new Error(`session cannot be loaded: ${message} (if another dsh surface is still using this session, close it before resuming)`);
	...
	return { showResume() {
		...
		const scanCandidates = async () => {
			...
			candidates.sort((a, b) => a.lastActivityAt - b.lastActivityAt || a.record.header.id.localeCompare(b.record.header.id));
...
var TuiOverlayManager = class {
	...
	* @param placement - terminal overlay for extensions, inline for the built-in question panel, or inline-below for the permission selector under the input box.
...
function createTuiChat(ctx, config, runtime, handle, selection) {
	...
	let reasoningMode = resolved.showReasoning ? "collapsed" : "hidden";
	let approvalPolicy = effectiveApprovalPolicy(agent.session.events) ?? ctx.get("approval")?.config?.policy ?? "ask";
	...
	const approvalCallByDecision = /* @__PURE__ */ new Map();
	...
	const updateInputBox = () => {
		const model = selection.current === void 0 ? void 0 : compactTargetLabel(selection.current);
		const permission = currentPermissionLabel();
		inputBox.setRightLabel(model === void 0 ? displayText(permission) : displayText(`${model} · ${permission}`));
	};
	...
	const shimmerActive = () => {
		...
		for (const card of allToolCards) if (card.isPending()) return true;
		return false;
	};
	const stopShimmerTimer = () => {
		if (shimmerTimer === void 0) return;
		clearInterval(shimmerTimer);
		shimmerTimer = void 0;
	};
	const updateShimmerTimer = () => {
		if (!shimmerActive() || disposed) {
			stopShimmerTimer();
			return;
		}
		if (shimmerTimer === void 0) shimmerTimer = setInterval(() => {
			requestRender();
		}, 120);
	};
	const requestRender = () => {
	...
	*/
	...
	const startAssistantStep = (position) => {
		streaming = new StreamingAssistantComponent(position, () => agent.session.events, stepTimingTracker, now, reasoningMode, palette, mdTheme);
	...
	const parsedTool = (event) => {
		...
		const card = new ToolCardComponent(event.data.name, parsed, ctx.tools.get(event.data.name, agent), resolved.maxToolOutputLines, resolved.maxDiffEditLength, palette, mdTheme, now);
			...
			case "tool/result": {
				...
				if (card === void 0) {
					card = new ToolCardComponent("tool", {
						value: {},
						valid: true
					}, void 0, resolved.maxToolOutputLines, resolved.maxDiffEditLength, palette, mdTheme, now);
					card.setVisibility(toolsVisibility);
					chat.addChild(card);
					allToolCards.add(card);
					accountResident(card, 512);
	...
	*/
	...
	const setReasoningMode = (mode) => {
		reasoningMode = mode;
		const set = (step) => {
			step.setReasoningMode(mode);
		};
	...
	const overlayManager = new TuiOverlayManager({
		...
		show: (component, options, placement) => {
			...
			if (placement === "inline-below") {
				const modal = new InlineModalComponent(component, resolved.permissionDialogWidth, resolved.permissionDialogMaxHeight);
	...
	const commands = createCommandController({
		...
		queuePermissionCommand: (raw) => {
			permissionController.queuePermissionCommand(raw);
		},
		showResume: () => {
			resume.showResume();
		},
...
async function attachSessionToWorkspace(ctx, sessionId, cwd) {
	...
	await (await registry.resolveByPath(cwd) ?? await registry.create(cwd)).attachSession(sessionId);
}
...
function buildResumeArgv(sessionId) {
...
*/
function handoffResume(sessionId, cwd) {
	return new Promise((_resolve, reject) => {
		const command = process.argv[0];
		if (command === void 0) {
			reject(/* @__PURE__ */ new Error("tui: cannot determine the node executable for resume handoff"));
			return;
		}
...
async function run(ctx, config, runtime) {
	...
	try {
		await attachSessionToWorkspace(ctx, String(handle.agent.session.id), handle.agent.session.header.cwd ?? process.cwd());
	} catch (error) {
