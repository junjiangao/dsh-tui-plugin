...
function createTuiChat(ctx, config, runtime, handle, selection) {
	...
	const updatePromptLine = () => {
		promptLine.setText(palette.dim(`${formattedCwd}${branch === void 0 ? "" : ` (${displayText(branch)})`}  ${agent.status === "running" ? "running" : "idle"}`));
	};
	updatePromptLine();
	const updateStatusLine = () => {
		...
		segments.push(`model ${selection.current === void 0 ? "unset" : compactTargetLabel(selection.current)}`);
...
async function run(ctx, config, runtime) {
	...
	const agentOptions = route === void 0 ? void 0 : {
