...
*/
function assistantMessageChildren(content, reasoningMode, foldedContinuation, palette, mdTheme) {
...
var StreamingAssistantComponent = class extends Container {
	...
	*/
	...
	rebuild() {
		this.clear();
		const children = assistantMessageChildren(this.presentedContent(), this.reasoningMode, this.foldedContinuation, this.palette, this.mdTheme);
...
var ToolCardComponent = class extends CachedCardComponent {
	...
	*/
	...
	renderLines(width) {
		...
		const header = truncateToWidth(`${statusColor(glyph)} ${isError ? this.palette.error(headerRest) : this.palette.dim(headerRest)}`, layout.innerWidth, "");
