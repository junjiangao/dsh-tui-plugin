...
*/
var BackgroundPanel = class extends Container {
	palette;
	paddingX;
	paddingY;
	constructor(palette, paddingX = 2, paddingY = 1) {
		super();
		this.palette = palette;
		this.paddingX = paddingX;
		this.paddingY = paddingY;
...
function assistantMessageChildren(content, reasoningMode, foldedContinuation, palette, mdTheme) {
	...
	if (showsReasoning) {
		...
		children.push(new Text(palette.italic(palette.dim(collapsed ? "▸ Thinking" : "▾ Thinking")), 0, 0));
		if (!collapsed) children.push(new Markdown(reasoning, 0, 0, mdTheme, {
...
var ToolCardComponent = class extends CachedCardComponent {
	...
	*/
	...
	renderLines(width) {
		...
		const lines = ["", truncateToWidth(`${statusColor(glyph)} ${this.palette.dim(headerText.slice(glyph.length + 1))}`, Math.max(1, width - 2), "")];
