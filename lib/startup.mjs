...
//#endregion
//#region packages/tui-app/src/startup.ts
/**
...
*/
function tuiCommand() {
	return new Command().name("dsh --profile tui").description("Run the interactive full-screen terminal coding agent.").helpOption("-h, --help", "show this help").option("--resume <sessionId>", "resume a persisted session instead of minting a fresh one").option("--session <sessionId>", "explicit id for the fresh session this invocation creates").option("--model <provider>/<model>", "provider/model route for this session, e.g. deepseek-official/deepseek-v4-pro").option("--tool-mode <native|code|both>", "tool presentation mode; overrides DSH_TOOLS_MODE and the schema default").option("--preset <id>", "compose this session from the named agent preset; absent mounts the roster default").addHelpText("after", `
Examples:
...
function apply(ctx) {
	...
	program.action(() => {
		if (!process.stdin.isTTY || !process.stdout.isTTY) program.error("error: the TUI requires stdin and stdout to be interactive TTYs; use `dsh --profile headless \"task\"` for pipes and automation");
		const options = program.opts();
		const toolMode = parseToolMode(options.toolMode);
		if (options.resume !== void 0 && options.session !== void 0) program.error("error: --resume and --session are mutually exclusive: a session is either resumed or freshly created");
		if (options.toolMode !== void 0 && toolMode === void 0) program.error(`error: --tool-mode must be native, code, or both, got ${JSON.stringify(options.toolMode)}`);
		if (options.model !== void 0 && !options.model.includes("/")) program.error(`error: --model must be <provider>/<model>, got ${JSON.stringify(options.model)}`);
		if (options.preset !== void 0 && !/^[a-z0-9][a-z0-9-]*$/u.test(options.preset)) program.error(`error: --preset must be a preset id ([a-z0-9][a-z0-9-]*), got ${JSON.stringify(options.preset)}`);
		const freshSessionId = options.resume === void 0 ? options.session ?? `session-${randomUUID()}` : void 0;
		ctx.provide(TUI_STARTUP_SERVICE, {
			...options.resume !== void 0 && { resumeSessionId: options.resume },
			...freshSessionId !== void 0 && { sessionId: freshSessionId },
