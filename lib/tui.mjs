...
var CachedCardComponent = class {
	...
	*/
	...
	render(width) {
		...
		if (this.cached?.width !== width || this.cached.epoch !== epoch) this.cached = {
			width,
			epoch,
...
function createPermissionController(deps) {
	...
	const currentSandboxMode = () => {
		...
		return ctx.shell?.sandboxMode ?? ctx.sandboxPolicy?.defaultMode ?? "read-only";
	};
...
function createTuiChat(ctx, config, runtime, handle, selection) {
	...
	let approvalPolicy = effectiveApprovalPolicy(agent.session.events) ?? ctx.approval?.config?.policy ?? "ask";
